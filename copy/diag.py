...
async def scan_pair(symbol: str, tf: str):
    ...
    try:
        async with aiohttp.ClientSession() as s:
            ...
            try:
                # Используем правильную сигнатуру
                ch.plot_png(candles, levels, out, title=f"{symbol} {tf}")
                print(f"{tag} PNG => {out} (size: {os.path.getsize(out) if os.path.exists(out) else 0})")
...
try:
    sig = inspect.signature(st.calculate_rsi)
    print("✔ calculate_rsi:", sig)
except Exception:
    print("❌ calculate_rsi недоступна")
    traceback.print_exc()
